...
(function (racine, fabrique) {
  ...
  function visuelDe(id, role, catalogue) {
    ...
    var r = role || (def && def.roles && def.roles[0]) || "assaisonnement";
    return PAR_ROLE[r] || PAR_ROLE.assaisonnement;
  }

  function plat(resultat, catalogue, categorie) {
    var visibles = resultat.ingredients.filter(function (i) {
      return i.statut !== "omis" && i.statut !== "impossible";
    }).map(function (i) {
      var id = i.vers || i.id;
    ...
    var fond = TEINTES_FOND[categorie] || TEINTES_FOND["Repas"];
    ...
    var mode = (base.forme === "arc") ? "liquide" : (base.forme === "chunk" ? "morceaux" : "dome");
    if (mode === "liquide") {
