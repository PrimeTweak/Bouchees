...
/* Traduit un événement Stripe en mise à jour de compte, ou null si l'événement
 * ne nous concerne pas. On ne s'intéresse qu'au cycle de vie de l'abonnement. */
function evenementPertinent(evt) {
  if (!evt || !evt.type || !evt.data || !evt.data.object) return null;
  const o = evt.data.object;

  const courriel = o.customer_email || o.customer_details && o.customer_details.email ||
                   o.metadata && o.metadata.courriel || null;
  const fin = o.current_period_end ? new Date(o.current_period_end * 1000).toISOString() : null;

  switch (evt.type) {
    case "checkout.session.completed":
      if (!courriel) return null;
      return { courriel: courriel, statut: "actif", finPeriode: fin, client: o.customer || null };
    ...
    case "customer.subscription.updated": {
      ...
      return { courriel: courriel, statut: carte[o.status] || "annule",
               finPeriode: fin, client: o.customer || null };
    }

    case "customer.subscription.deleted":
      if (!courriel) return null;
      return { courriel: courriel, statut: "annule", finPeriode: fin, client: o.customer || null };

    case "invoice.payment_failed":
      if (!courriel) return null;
      return { courriel: courriel, statut: "en_retard", finPeriode: fin, client: o.customer || null };
...
async function creerSession(options) {
  ...
  const corps = new URLSearchParams({
    ...
    customer_email: options.courriel,
    "metadata[courriel]": options.courriel,
    "subscription_data[metadata][courriel]": options.courriel,
