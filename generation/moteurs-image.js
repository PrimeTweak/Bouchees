  ...
  generer: async function (spec) {
    ...
    const rep = await fetch(base + "/sdapi/v1/txt2img", {
      ...
      body: JSON.stringify({
        ...
        width: spec.largeur || 1024,
        height: spec.hauteur || 768,
    ...
    return { octets: Buffer.from(d.images[0], "base64"),
             largeur: spec.largeur || 1024, hauteur: spec.hauteur || 768, moteur: "drawthings" };
  }
};
...
function choisir(nom) {
  const cible = nom || process.env.MOTEUR_IMAGE || "";
