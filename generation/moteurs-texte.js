...
const simule = {
  ...
  rediger: async function (prompt) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const liant = par("liant")[i % Math.max(par("liant").length, 1)] || ids[1];
      const assais = par("assaisonnement")[i % Math.max(par("assaisonnement").length, 1)] || ids[2];
      out.push({
        id: "simule-" + categorie.toLowerCase() + "-" + (i + 1),
        nom: "Recette simulée " + (i + 1),
        categorie: categorie,
        portions: "4 portions",
        ageMinBase: ageMinBase,
        tempsMin: 15,
        ingredients: [
          { id: base, qte: 250, unite: "ml" },
          { id: liant, qte: 125, unite: "ml", role: "liant" },
          { id: assais, qte: 2, unite: "ml" }
        ],
        etapes: [
...
const SIMULE_ROLES = {
  compote_pommes: "liant", puree_banane: "liant", lin_moulu: "liant", graines_chia: "liant",
  banane: "fruit", pomme: "fruit", mangue: "fruit", bleuets: "fruit",
  cannelle: "assaisonnement", vanille: "assaisonnement", jus_citron: "assaisonnement"
};

const MOTEURS = { anthropic: anthropic, openai: openai, simule: simule };

function choisir(nom) {
  const cible = nom || process.env.MOTEUR_TEXTE || "";
