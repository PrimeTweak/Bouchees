...
## Structure
    ...
    data/base.json           Familles d'allergènes (liste canadienne, 11),
                                stades de texture par âge, règles d'interdits
                                et de préparations par âge
    data/ingredients.json    ~65 ingrédients canoniques : allergènes dérivés,
                                rôles fonctionnels (liant, gras, farine, liquide…)
    data/substitutions.json  Table de substitution par (ingrédient × rôle),
                                options ordonnées par priorité, ratio, âge minimum
    data/recipes.json       20 recettes témoins couvrant les allergènes majeurs
    data/imported/          Recettes importées par le pipeline + rapport (générés)
    engine/engine.js            Le moteur (lot 2) — fonctions pures, zéro dépendance,
                                UMD (node + navigateur)
    ingestion/                  Lot 3 — pipeline d'ingestion agnostique de la source :
      ...
      importer.js                 portes de sécurité + rapport : node ingest/importer.js
    ...
    demo/template.html           Gabarit du banc d'essai technique (source)
    demo/build.js               Build : node demo/build.js → demo/index.html
    demo/index.html             Banc d'essai autonome (généré)
    web/template.html        Gabarit de l'app parents (source)
    app/illustration.js         Système visuel — illustration SVG générée à partir
                                des ingrédients ADAPTÉS + glyphes d'allergènes
    web/build.js                Build : node web/build.js → web/index.html
    web/index.html              L'app parents autonome (générée — un seul fichier)
    tools/gaps.js             Bloc B — où manquent les recettes (profil × âge)
    tools/publish.js           Bloc A — écrit dist/ : manifeste + lots versionnés
    tools/gap-report.md     Dernier rapport (généré)
    generation/recipe-prompt.js  Bloc C — prompt contraint au catalogue
    generation/recipe-validator.js Bloc C — rejette tout ingrédient inventé
    ...
    server/server.js          Bloc F — API, droits, service filtré (Node pur)
    server/stripe.js           Bloc F — signature webhook + session de paiement
    data/publishing.json    Bloc A — quel lot publie quoi, et à quel accès
    dist/                       Sortie de publication servie par le serveur
    tools/cycle.js             LE cycle du mois en une commande
    generation/text-engines.js Adaptateurs de rédaction (anthropic/openai/simulé)
    generation/image-engines.js Adaptateurs d'image (Draw Things/openai/simulé)
    ...
    server/apple.js            Bloc I — vérification StoreKit 2 (JWS, chaîne x5c)
    engine/native-bridge.js        Surface JSON appelée par JavaScriptCore
    ...
    tools/check-swift.py    Vérificateur statique — à défaut de compilateur
...
les valide. Licences des API commerciales : voir `ingest/LICENSES.md` —
...
NOUVEAUX (`git add`) :
    ...
    ingest/lexicon.json          ingest/normalizer.js
    ingest/adapters.js        ingest/importer.js
    ingest/curation.json         ingest/LICENSES.md
    ingestion/rapport-import.md     ingest/sources/partenaire-qc.json
    ingest/sources/spoonacular-fixture.json
    ingest/sources/mealdb-fixture.json
    data/imported/imported-recipes.json
    data/imported/rapport-import.json
    web/template.html            web/build.js            web/index.html

MODIFIÉS :

    data/base.json               data/ingredients.json
    data/substitutions.json      tests/test.js
...
`node tools/cycle.js` fait tout : trous → prompt → rédaction → validation →
...
NOUVEAUX (`git add`) :
    ...
    engine/native-bridge.js              tools/check-swift.py
...
vérificateur statique (`tools/check-swift.py`) attrape les symboles
...
NOUVEAUX (`git add`) :
    ...
    server/apple.js                  ios/App/App/BoucheesApp.swift
...
MODIFIÉS :
    ...
    server/server.js     (routes Apple + consultation de produits)
    web/template.html   (pont natif, abonnement iOS → StoreKit)
    web/index.html         (reconstruit)
...
NOUVEAUX (`git add`) :
    ...
    tools/cycle.js                   generation/text-engines.js
    generation/image-engines.js       generation/vision.js
    generation/coherence.js

MODIFIÉS :

    generation/images.js   (la révision automatique compte, avec verdict joint)
    web/template.html   (photo si telle quelle, illustration sinon)
    web/index.html         (reconstruit)
...
NOUVEAUX (`git add`) :
    ...
    data/publishing.json          tools/gaps.js
    tools/publish.js                 tools/gap-report.md
    tools/gap-report.json         generation/recipe-prompt.js
    generation/recipe-validator.js    generation/images.js
    generation/monthly-prompt.txt     generation/images/to-generate.json
    generation/images/manifest.json  server/server.js
    server/stripe.js                 MARCHE-A-SUIVRE.md
...
MODIFIÉS :
    ...
    web/template.html   (paywall, lots, nouveautés, mode serveur)
    web/build.js           (injection de la publication)
    web/index.html         (reconstruit)
...
À ignorer dans Git : `server/accounts.json` (données de comptes).
...
MODIFIÉS :
    ...
    web/template.html   (refonte complète)
    web/build.js           (injection du système visuel)
    web/index.html         (reconstruit)
